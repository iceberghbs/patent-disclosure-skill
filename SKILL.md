...
- **图示定稿（Step 7）**：**5.1**/**5.2** 用 fenced **mermaid**（**须** B/W 主题）；执行方式、**`mmdc`** 安装与降级规则见下表「交底书定稿交付」行及 **`tools/README.md`**。
...
| 交底书定稿交付（**须同时** .md + .docx） | **5.1** 系统框图与 **5.2** 流程图均用 fenced ``mermaid``（**须** B/W 主题），**不要** ASCII 文字流程图/框图。定稿执行 **`tools/build_docx.py`**：mermaid 转 PNG（B/W）→ pandoc 生成含 OMML 原生公式的 `.docx`。详见 **`tools/README.md`** |
...
| Step 7 | `prompts/disclosure_builder.md` + `prompts/template_reference.md` | 交底书结构、脱敏、**符号与公式体例（§7.7）**与图示规范；**mermaid 与 5.2.1 符号/公式范例在 template_reference** |
...
- **补充材料 / 扩展章节**或 **§7.6 第七章权利要求书式强化（用户已声明侧重点）**：`Read` → `iteration_context.md` → `merger.md`；合并结果**另存为**带时间戳的 `.md`/`.docx`（§7.3 第 5 点）；**追加** `交底书修订对话记录.md`（`iteration_dialog_log.py` 或手工）；完成后**必须**输出「合并摘要」留档；若本轮亦为定稿交付，**仍建议**简短附带 §7.6 类引导  
...
□ 查新完成且写入 3.1 与区别论述（符合 `prior_art_search.md`：**优先** `tools/cnipa_epub_search.py`，**国知局侧已分多次调用、每轮一词，并已自行合并** `EPUB_HITS_JSON`；**`abstract` 必用且已充分理解后再概括**；异常或无果再 **WebSearch**）
□ 除用户明确跳过外，完成摘要预览
□ 脱敏、mermaid（定稿均已渲染为 B/W PNG）、章节引用符合 template_reference；含公式时 **5.2.1 符号表、§7.7 体例**（维度下标、无字母多义、LaTeX 分隔符统一）及 **5.2.2 符号列同形** 已满足；**已交付 .md 与 .docx**，且**文件名符合 §7.3 第 5 点**（**凡交付均含**时间戳后缀）；**正文无**技能/示例仓库类文末脚注
