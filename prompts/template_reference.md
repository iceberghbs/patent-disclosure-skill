...
> 检索渠道、链接格式与摘要要求以 Step 5 `prior_art_search.md` 为准。检索渠道信息仅供 Agent 内部记录，**不写入交底书正文**（见 `disclosure_builder.md` §7.5.3）。查新检索结果使用 `reference/search_report_template.doc` 模板单独呈现。
...
- 技术细节以第四章为准，本章以论点为主
- 请从技术角度描述有益效果
- 如果技术手段本身是已知的，但技术手段的特定结合能够带来超越各手段单独使用时的有益效果，应在效果描述中自然体现这种组合协同，不单独成节、不显式标注"1+1>2"（见 `disclosure_builder.md` §7.5.2）
