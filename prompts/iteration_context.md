...
用户明显在**已有交底书或上一轮交付稿**上继续工作时（补材料、改章节、纠错、调发明点表述等），在 **`Read` `merger.md` 或 `correction_handler.md` 之前**先读本文，再读对应迭代模板。

| 意图 | 下一步模板 |
|------|------------|
| 补充文档、扩展方案、合并新材料 | `merger.md` |
| 指出错误、与事实/参数不符、风格或发明点调整 | `correction_handler.md` |
| 用户已按 `disclosure_builder.md` §7.6 声明侧重点，仅需 **第三章发明点概述式强化**（取向须与本稿已有材料及第三、四章已写观点一致，**禁止**为交互而编造新场景） | `merger.md`（以最近定稿为基准，合并范围以第三章为主，必要时微调第五章与第三章衔接句） |
...
- 合并或纠正后的全文写入**新文件**，不得默认覆盖旧稿：  
  **`{规范化交底书名}_{YYYYMMDDHHmmss}.md`** + **`build_docx.py` 生成的同名 `.docx`**。  
...
```text
python3 ${CLAUDE_SKILL_DIR}/tools/iteration_dialog_log.py --case-dir "{案件目录}" --kind merge --user "{用户说明摘要}" --summary "{摘要摘录}" --artifacts "{交底书名_时间戳.md},{交底书名_时间戳.docx}"
```

`--kind` 纠正时用 `correct`。若无法执行脚本，须 **`Read`** 已有 `交底书修订对话记录.md`（若无则 **`Write`** 创建），再 **`Edit`** 或等价方式在文末**追加**与上表结构相同的一条（时间须真实）。
...
4. **`Write`** 新时间戳 `.md` → 运行 **`build_docx.py -o`** 写出定稿图与 **`.docx`**。
