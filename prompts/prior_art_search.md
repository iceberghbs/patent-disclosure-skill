...
生成交底书全文**之前或生成过程中**必须执行；检索结论写入**第一章（1. 背景技术）** 及与本案的**区别论述**。
...
4. **执行方式**（Step 5 在读完本文件后**先尝试**）：
   ...
   - **合并责任在 Agent**：每次调用解析 **stdout** 上**唯一一行** **`EPUB_HITS_JSON:`** 后的 JSON 数组；在推理中按 **`pub_number`** 为主键去重合并（无则 **`link`**，再否则可用标题前缀），得到**一份**总表后再写入查新笔记与 1。
   ...
   - 将 JSON 中**可核验**的公开号、标题、**国知局站点内详情链接**写入查新笔记与 1（见下 **`abstract` 必用**）。
...
5. **`abstract` 字段（国知局条目，规定必用）**
   ...
   - **必用**：查新笔记、交底书 **1** 中对该专利的**技术方案概括、应用场景与局限性分析**，**必须先基于对该 `abstract` 的完整阅读与理解**后再撰写；**禁止**仅凭标题、公开号或 URL **臆造**方案要点或与摘要矛盾的表述。
   - **充分理解**：在写入 1 或查新笔记前，Agent 须在**推理过程内**明确：摘要所涉**技术领域、解决什么问题、核心手段/模块、主要效果或流程**；若摘要与标题存在差异，**以摘要为准**概括该技术。
   - **正文呈现**：交底书 1 中**不得**大段逐字粘贴官方摘要（避免抄袭与超字数）；应**消化后**用**自己的话**压缩为「方案概括 + 应用 + 缺点/局限」；查新笔记可保留稍长的摘录供自用核对，但须标注来源于公布站摘要。
   - **缺失时**：若某条 JSON **无** `abstract` 或为空（旧版页面 / 表格布局未解析到等），须在查新笔记中注明「该条无摘要字段」，并改用**详情页**或 **Google Patents** 等可核验来源补全理解后再写 1，**不得**留空理由含糊带过。
...
- 应用场景
- **局限性**
- **公开源 URL（必填）**：每一条必须附带**至少一个可公开访问、与著录项一致**的链接，写入查新笔记与交底书 1，便于代理人复核。**禁止编造或猜测 URL**；写入前应在浏览器中打开确认页面可访问且对应同一文献/专利。
...
文末给出：**检索总结**与**本发明与现有技术的本质区别**，与 1 结尾及 2（所要解决的问题）呼应。
...
- **交底书正文（1. 背景技术）不包含「检索说明」段**：检索渠道、检索词、检索工具等流程信息仅供 Agent 内部记录，**不写入**交付正文。背景技术节直接从技术背景和现有技术实现方案开始。
- **查新检索结果单独呈现**：检索完成后，查新结论、对比文件分析、新颖性/创造性评价等使用 **`reference/search_report_template.doc`** 模板单独生成查新检索报告，不写入交底书正文。交底书背景技术节中的现有技术引用仅保留：专利号、技术方案概括、局限性、公开源 URL。

查新笔记（Agent 内部或对话留档）仍可记录是否调用脚本、是否降级 WebSearch；**上述内容不得原样抄进交底书正文**。
